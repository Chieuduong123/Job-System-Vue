...
import RedTick from "./RedTick.vue";
    ...
    const accessToken = localStorage.getItem('tokenBusiness')
    const {onToggleUpdate, idTemp} = defineProps({
        onToggleUpdate: Function,
        idTemp: String
    }) 

    const handleGetPostById = async(idPost, token) => {
        await postStore.actGetPostByIdForBusiness(idPost, token)
    }

    onMounted(async() => {
        handleGetPostById(idTemp, accessToken)
    })
