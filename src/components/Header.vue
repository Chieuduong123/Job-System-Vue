            ...
            <div v-if="userStore?.isLogged" class="flex items-center gap-5 max-sm:gap-2">
                ...
                <Menu as="div" class="relative z-20 inline-block text-left">
                    <MenuButton
                        class="w-[40px] h-[40px]"
                        >
                        <img v-if="handleCheckLinkImage(userStore?.myUser?.avatar)" :src="userStore?.myUser?.avatar ? `${userStore?.myUser?.avatar}` : 'https://e7.pngegg.com/pngimages/178/595/png-clipart-user-profile-computer-icons-login-user-avatars-monochrome-black-thumbnail.png'" alt="avatar" class="w-full h-full object-cover rounded-full">
                        <img v-else :src="userStore?.myUser?.avatar ? `${IMAGE_URL}${userStore?.myUser?.avatar}` : 'https://e7.pngegg.com/pngimages/178/595/png-clipart-user-profile-computer-icons-login-user-avatars-monochrome-black-thumbnail.png'" alt="avatar" class="w-full h-full object-cover rounded-full">
                    </MenuButton>
                    <transition
